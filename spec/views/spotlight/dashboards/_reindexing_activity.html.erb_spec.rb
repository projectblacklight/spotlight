describe 'spotlight/dashboards/_reindexing_activity.html.erb', type: :view do
  ...
  let(:recent_reindexing) do
    [FactoryBot.build(:unstarted_job_log_entry)] + # nil start_time is trouble for the sort_by used to create the rest of the fixture's rows
      [
        FactoryBot.build(:job_log_entry),
        FactoryBot.build(:in_progress_job_log_entry),
        FactoryBot.build(:recent_job_log_entry),
        FactoryBot.build(:failed_job_log_entry)
  ...
  context 'a reindexing log entry has a null user' do
    it 'displays blank in the user field and renders without error' do
      assign(:recent_reindexing, [FactoryBot.build(:job_log_entry_no_user)])
