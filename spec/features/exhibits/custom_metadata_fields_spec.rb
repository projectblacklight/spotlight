...
RSpec.describe 'Adding custom metadata fields', type: :feature do
  ...
  it 'allows admins to manage a new custom field' do
    ...
    within '#exhibit-specific-fields' do
      ...
      expect(page).to have_css '.field-description', text: 'Helps to remind me what this field is for'
      # Edit
      click_link 'Edit'
    end
    ...
    within '#exhibit-specific-fields' do
      expect(page).to have_css '.field-label', text: 'My new custom field'
      expect(page).to have_css '.field-description', text: 'A much better description'
      # Destroy
      click_link 'Delete'
    end
