...
export default class {
  connect(){
    ...
    if($('[data-featured-image-typeahead]').length > 0) {
      $('[data-featured-image-typeahead]').spotlightSearchTypeAhead({bloodhound: this.itemsBloodhound(), template: this.itemsTemplate()}).on('click', function() {
        $(this).select();
      }).on('typeahead:selected typeahead:autocompleted', function(e, data) {
        var panel = $($(this).data('target-panel'));
        new spotlightAdminAdd_image_selector($(this), panel, data.iiif_manifest, true).connect()
