    ...
    def perform(exhibit, bulk_update)
      ...
      File.open(csv_path) do |f|
        ...
        ::CSV.table(f, header_converters: header_converter).each do |row|
          ...
          added_tags_cols, removed_tags_cols = row.headers.select { |k| k =~ /^#{config.csv_tags_prefix}/ }.partition do |k|
            ActiveModel::Type::Boolean.new.cast(row[k])
          end

          added_tags = added_tags_cols.map { |x| x.sub(/#{config.csv_tags_prefix}\s/, '') }
