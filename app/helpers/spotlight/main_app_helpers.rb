  ...
  module MainAppHelpers
    ...
    def exhibit_stylesheet_link_tag(tag)
      if current_exhibit_theme && current_exhibit.theme != 'default'
        ...
        Rails.logger.warn "Exhibit theme '#{current_exhibit_theme}' not in white-list of available themes: #{Spotlight::Engine.config.spotlight.exhibit_themes}"
        stylesheet_link_tag(tag)
      end
    end

    def current_exhibit_theme
      current_exhibit.theme if current_exhibit && current_exhibit.theme.present? &&
                               Spotlight::Engine.config.spotlight.exhibit_themes.include?(current_exhibit.theme)
    end
  end
end
