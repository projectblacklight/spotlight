    ...
    class Csv < Spotlight::Resource
      ...
      def label_to_field
        @label_to_field ||= begin
          ...
          label_to_field.merge! Hash[exhibit.blacklight_config.index_fields.map { |_k, v| [v.label, v.field] }]
          label_to_field.merge! Hash[exhibit.blacklight_config.facet_fields.map { |_k, v| [v.label, v.field] }]
          label_to_field
        end
      end

      def csv(options = { headers: true }, &_block)
      ...
      def update_exhibit_specific_fields
        csv.map do |row|
          sidecar_updates = {}
          row.each do |label, v|
            sidecar_updates[custom_fields[label]] = v if custom_fields[label]
          end

          unless row[public_field_name].blank? && sidecar_updates.empty?
